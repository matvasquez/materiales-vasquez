...
export default async function getNewProductsGarden(req, res) {
  // Run cors
  await cors(req, res);
  ...
  setTimeout(async () => {
    const result = await rest.executeQuery(
      ...
      WHERE a.DESC_BREVE LIKE 'LAMPARA%' AND a.HABVTAS = '' AND l.NO_LISTAP = '001' AND i.IMAGEN Is NOT NULL AND s.CVEALM IN ('0020','0007','0018','0014','0015','0002','0008','0023','0017','0028','0027') 
    ...
    result &&
      res.send(
        res.status(200).json({
          name: "Products Secction by Name",
          method: req.method,
          total: result.data[0].length,
          productsByName: result.data[0],
        })
      );
  }, 1000);
}
