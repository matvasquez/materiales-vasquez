  ...
  setTimeout(async () => {
    const result = await rest.executeQuery(
      `SELECT TOP 8 ROW_NUMBER () OVER(ORDER BY a.FECHA_ALTA DESC) AS row_id, RTRIM(a.CLAVEART) AS articulo_id, RTRIM(a.DESC_BREVE) AS name, RTRIM(a.DESCRIBEAR) AS description, l.PREC_IVA1 AS price, RTRIM(a.CVE_CLAS) AS CVE_CLAS, RTRIM(c.DESCRIBECO) AS category, RTRIM(a.CLAVEGIR) AS CLAVEGIR, RTRIM(g.DESGIR) AS main_category, cast('' as xml).value(
        ...
        WHERE l.PREC_IVA1 < 200 AND a.HABVTAS = '' AND l.NO_LISTAP = '001' AND i.IMAGEN Is NOT NULL AND s.CVEALM IN ('0020','0007','0018','0014','0015','0002','0008','0023','0017','0028','0027') 
